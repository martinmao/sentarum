...
public interface TagEntityMapper extends ModelMapper<TagEntity, TagModel> {

    default TagGroup toTagGroup(TagGroupEntity entity) {
        if (!isEntityInitialized(entity)) {
            return null;
        }
        return (TagGroup) ModelMapperRepository.getRequiredModelMapper(TagGroupEntityMapper.class).mapForRead(entity);
    }

    default TagGroupEntity toTagGroupEntity(TagGroup tagGroup) {
        return null;
    }
}
