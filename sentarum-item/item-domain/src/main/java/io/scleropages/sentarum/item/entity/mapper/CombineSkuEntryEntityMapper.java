...
public interface CombineSkuEntryEntityMapper extends ModelMapper<CombineSkuEntryEntity, CombineSkuEntryModel> {

    default SkuEntity toSkuEntity(Sku sku) {
        return null;
    }
    ...
    default Sku toSku(SkuEntity entity) {
        if (!isEntityInitialized(entity))
            return null;
        return (Sku) ModelMapperRepository.getRequiredModelMapper(SkuEntityMapper.class).mapForRead(entity);
    }
